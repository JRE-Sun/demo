class ZeptoMap {
    constructor({openCallBack = null, shutCallBack = null} = {}) {
    ...
    shutGetAddress() {
        let $addressSelector = '';
        let inputControl     = $('#address').parents('.input-control');
    ...
    mapSelector() {
        let map       = new BMap.Map("map");
        // 创建地址解析器实例
        let initPoint = new BMap.Point(116.331398, 39.897445);
        map.centerAndZoom(initPoint, 18);
        let marker = new BMap.Marker(initPoint);
        ...
        let topRightNav = new BMap.NavigationControl({
